...
export default function CrearProyecto() {
  ...
  const handleAgregarMasRequisitos = (agregar: boolean) => {
    if (agregar) {
      ...
      // Si no quiere agregar más requisitos, guardar el puesto completo y mostrar pantalla de agregar más puestos
      if (puestoActual) {
        const puestoCompleto = {
          ...puestoActual,
          requisitos: [
            ...(puestosCreados.find((p) => p.codPuesto === puestoActual.codPuesto)?.requisitos || []),
            {
              codCarrera: carreraFormData.codCarrera,
              nombreCarrera: getCarreraNombre(carreraFormData.codCarrera),
              cantMateriasAprobadasReq: Number.parseInt(carreraFormData.cantMateriasAprobadasReq),
        ...
        // Actualizar o agregar el puesto en la lista
        setPuestosCreados((prev) => {
          const existingIndex = prev.findIndex((p) => p.codPuesto === puestoActual.codPuesto)
          if (existingIndex >= 0) {
            const updated = [...prev]
            updated[existingIndex] = puestoCompleto
            return updated
          } else {
            return [...prev, puestoCompleto]
          }
        })
      }
